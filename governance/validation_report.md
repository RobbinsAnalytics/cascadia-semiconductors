...
*Auto-generated by `src/validate.py` on 2026-07-22. Data snapshot as-of **2026-07-21** (SEC EDGAR XBRL, frozen in `data/raw/`).*
...
| 7 | Peer FY reconciliation (each peer's quarters sum to its filed FY) | PASS | 120 peer metric-years reconciled |
| 8 | Peer gap audit (missing / not-comparable is flagged, never silent) | PASS | 2178 peer rows scanned; 712 flagged missing |
...
Peer `companyfacts` bundles were pulled by `src/ingest_peers.py` as an ADDITIVE
extension of the 2026-07-21 freeze; **actual retrieval date 2026-07-22**. The
benchmark never shows a peer quarter later than FORM's frozen latest (**2026Q1**), so the later pull cannot leak newer data. Peers are conformed
with the same rules as FORM (`src/conform_peers.py`).

**Snapshot quarter for the ranked comparison: 2026Q1.** Availability of each
...
- **CAMT (Camtek)** — foreign private issuer; files Form 20-F/6-K, not 10-Q/10-K. No recent quarterly GAAP XBRL, so it is excluded from every quarterly ranking.
- **KLAC (KLA)** — tagged `GrossProfit` only through FY2021 Q2 and does not tag `OperatingIncomeLoss` in-window (latest 2015), so gross and operating margin are not comparable at the snapshot; shown on revenue scale, R&D %, and growth (it is a process-control margin *reference*, not a direct comp).
