...
| Verification | [`governance/validation_report.md`](governance/validation_report.md), auto-generated by `validate.py`: FY reconciliation, cross-foot identity (including divestiture gains), gap audit, press-release spot-check table |
...
python src/build_db.py      # tidy CSV  -> SQLite star schema
python src/validate.py      # checks    -> governance/validation_report.md (exit 1 on failure)
python src/build_pages.py   # tidy CSV  -> docs/ JSON + inline data injection
```

`python src/ingest.py` re-pulls from SEC and **overwrites the freeze** — only
run it to deliberately refresh the dataset.
...
- **Phase B (planned):** gross-margin drivers and segment disclosures
  (Probe Cards vs Systems), to the extent GAAP filings disclose them.
